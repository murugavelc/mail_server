...
function fit_string_to_size(a,b,d){var c=a;rcmail.env.tmp_span?b=rcmail.env.tmp_span:(b=$("<b>").css({visibility:"hidden",padding:"0px","font-family":b.css("font-family"),"font-size":b.css("font-size")}).appendTo($("body",document)).get(0),rcmail.env.tmp_span=b);var f=$(b);f.text(c);var e=b.offsetWidth;if(e>d){d=Math.max(1,Math.floor((e-d)/e*a.length/2));for(var c=Math.floor(a.length/2),g;;){e=c-d;g=c+d;f.text(a.substring(0,e)+"..."+a.substring(g));if(3>e||b.offsetWidth)break;d++}c=a.substring(0,
