...
"body"==f?"block":"none",d&&(d.style.display="message"==f?"block":"none"));r&&(r.style.display="currentdate"==f||"date"==f?"inline":"none");t&&(t.style.display="date"==f?"":"none");$('[value="exists"],[value="notexists"]',n).prop("disabled","string"==f);n.val()&&!n.val().match(/^(exists|notexists)$/)||n.val("contains");rule_op_select(e,a,f);rule_mod_select(a,f);b.style.width="..."==f?"40px":""}
...
function sieve_formattime(a,b){var c,d="",e=rcmail.env.time_format||"H:i";for(c=0;c<e.length;c++){var g=e.charAt(c);switch(g){case "a":d+=12<a?"am":"pm";break;case "A":d+=12<a?"AM":"PM";break;case "g":case "h":d+=("h"==g&&10>a?"0":"")+a;break;case "G":d+=a;break;case "H":d+=(10>a?"0":"")+a;break;case "i":d+=(10>b?"0":"")+b;break;case "s":d+="00";default:d+=g}}return d}
